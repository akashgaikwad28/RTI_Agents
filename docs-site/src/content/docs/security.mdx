---
title: Security & RBAC
description: Authentication, Authorization, and Security in RTI Agent.
---
...
## Multi layered Security Topology
...
## Operational Guardrails & Trust validation
...
### Policy Enforcement & Risk Validation
In the `consensus_node`, the system calculates a consolidated risk score. If the score exceeds the safety threshold (`risk_score > 0.65`) or if the quality reviewer flags multiple hallucinations, the system marks the request for escalation (`escalation_required = True`), forcing a manual human in the loop validation review.

## Rate Limiting & Auth Lifecycle

* **Rate Limits**: 
  * **API Key Limit**: 60 requests per minute.
